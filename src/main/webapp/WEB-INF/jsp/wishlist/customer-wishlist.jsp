...
<script>
	 ...
	 var datatable =  $('table#sampleTable').DataTable({
		...
		columns: [
...

                			{
                				...
                				render: function (data) {
                			//	console.log(data)
                					return '<div><a href="javascript:void(0);" class="btn btn-secondary btn-sm ms-5 rounded-1"  onclick="showProductDetailsModal(' + data + ');">Add to Card</a>  <a href="#" class="close-btn ms-3" onclick="deleteProductFromCart(' + data + ');"><i class="fas fa-close"></i></a></div>'
                				}
	...
	function deleteProductFromCart(id){
            $.ajax({
                url: '/ajax/deleteProductFromCart/' + id,
                type: 'DELETE',
                success: function(response) {
                    notifyMe('success', "Product removed from wishlist.");
