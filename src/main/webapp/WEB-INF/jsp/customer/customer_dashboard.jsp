              ...
              <div class="col-xl-9">
                <div class="container">
                    <div class="recent-orders bg-white rounded py-5">
                        <h6 class="mb-4 px-4">Orders History</h6>
                        <div class="table-responsive">
                            <table id="sampleTable" class="order-history-table table">

                              <thead style="background-color:#FFFFFF; font-weight: bold; justify-content: center;">
                                           <tr>
                                        <th  style="font-weight: bold; color:#5d6374; justify-content: left; ">Order Code</th>
                                        <th style="font-weight: bold; color:#5d6374; justify-content: left; ">Placed On</th>
                                         <th style="font-weight: bold; color:#5d6374; justify-content: left;">Items</th>
                                        <th style="font-weight: bold; color:#5d6374; justify-content: left; ">Total</th>
                                        <th  style="font-weight: bold; color:#5d6374; justify-content: left; "> Status</th>
                                         <th  style="font-weight: bold; color:#5d6374; justify-content: left; "> Action</th>
                                           </tr>
                                        </thead>
                            </table>
                        </div>
...
$(document).ready(function() {
    // Remove the form-control class from search input elements
    $('.dataTables_filter input[type="search"]').removeClass('form-control');
    // Add margin bottom
        $('.dataTables_filter input[type="search"]').css('margin-bottom', '20px');
});



   var datatable = $('table#sampleTable').DataTable({
      ajax: '/apidataTable/getUserOrders',
      serverSide: true,
      iDisplayLength: 10,
      order: [[ 1, "desc" ]],
      columns: [
...

         {
            ...
            render: function (data) {
                   ...
                   date =  d.toDateString() + ", " + d.getHours() + ":" + d.getMinutes() ;
                       ...
                       return '<div class="badge bg-primary" style="width:100%">' + date + '</div>'
...

         {
             data: 'id',
             render: function (data) {
                return '<div class="text-center"><a href="${pageContext.request.contextPath}/customer-invoice/${"' + data + '"}"  class="view-invoice fs-xs"  data-bs-toggle="tooltip" data-bs-placement="top"  data-bs-title="View Details"><i class="fas fa-eye"></i></a> </div>'
             }
          }
      ]
   });
...
// Hide the "Show entries" dropdown
    ...
    // Select all rows in the DataTable
        var rows = $('table#sampleTable tbody tr');

        // Set the height for each row
        rows.each(function() {
            $(this).css('min-height', '500px'); // Adjust the height as needed
        });
</script>