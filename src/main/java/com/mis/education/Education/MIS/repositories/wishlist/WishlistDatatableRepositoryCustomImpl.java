...
@Service
public class WishlistDatatableRepositoryCustomImpl implements WishlistDatatableRepositoryCustom {

    @PersistenceContext
    EntityManager entityManager;

    @Override
    public List<WishlistEntity> getProductViaId(Integer productId, Long userId) {
        ...
        Predicate transactionId_ = cb.equal(book.get("productId").get("productId"), productId);
        Predicate userId_ = cb.equal(book.get("userId").get("userId"), userId);
        cq.where(transactionId_,userId_);
        TypedQuery<WishlistEntity> query =  entityManager.createQuery(cq);
    ...
    public void deleteRow(Integer id) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaDelete<WishlistEntity> delete = cb.createCriteriaDelete(WishlistEntity.class);
        Root<WishlistEntity> root = delete.from(WishlistEntity.class);
        Predicate predicate = cb.equal(root.get("productId").get("productId"), id);
