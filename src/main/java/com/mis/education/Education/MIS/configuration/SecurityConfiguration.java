...
import com.mis.education.Education.MIS.captcha.CustomAuthenticationProvider;
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
       ...
       // http.csrf() .csrfTokenRepository(csrfTokenRepository()) .and().addFilterAfter(csrfHeaderFilter(), CsrfFilter.class);
        ...
        http.csrf().ignoringAntMatchers("/nocsrf", "/**");
...
//                .antMatchers("/createrole/").hasAnyRole("ADMIN")
    ...
    private AuthenticationSuccessHandler loginSuccessHandler() {
       // System.out.println(ServletUriComponentsBuilder.fromCurrentContextPath().path(Constants.indexController).toUriString());
    ...
    private LogoutSuccessHandler logoutSuccessHandler() {
        return (request, response, authentication) -> {
            ...
            response.sendRedirect(ServletUriComponentsBuilder.fromCurrentContextPath().path(Constants.homeController).toUriString());
        };
    }
