...
public class CustomAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication)
        ...
        System.out.println(encoder.encode(decodedPassword));

        boolean isPasswordMatch = encoder.matches(decodedPassword, userDetails.getPassword());

        UsernamePasswordAuthenticationToken token = null;
        if(isPasswordMatch) {
            token = new UsernamePasswordAuthenticationToken(userDetails,
                    userDetails.getPassword(), userDetails.getAuthorities());
        }else{
            throw new InsufficientAuthenticationException("Password Mismatch!");
        }
